...
async function scrapeSearch(query, apiBaseUrl) {
    try {
        ...
        $('.bgei').each((i, element) => {
            ...
            if (!title) {
                 title = $(element).find('.kan h3').text().trim() || $(element).find('.kan a').first().text().trim();
            }

            const link = $(element).find('a').attr('href');
            const thumbRaw = $(element).find('img').attr('data-src') || $(element).find('img').attr('src');
            const description = $(element).find('p').text().trim() || $(element).find('.kan p').text().trim();
            const type = $(element).find('.tpe1_inf').text().trim().split(' ')[0] || $(element).find('.tpe1 b').text().trim() || 'Unknown';
