const axios = require('axios');
const qs = require('qs');
const { wrapper } = require('axios-cookiejar-support');
const { CookieJar } = require('tough-cookie');

/**
 * Gemini AI V2 - BatchExecute Method
 * Automated Cookie Handling & Firebase Persistence
 */

const FIREBASE_URL = 'https://puru-tools-default-rtdb.firebaseio.com/cookieGeminiV2.json';

async function syncFirebase(method = 'GET', data = null) {
    try {
        if (method === 'GET') {
            const res = await axios.get(FIREBASE_URL);
            return res.data?.cookies || null;
        } else {
            await axios.put(FIREBASE_URL, {
                cookies: data,
                last_updated: new Date().toISOString()
            });
        }
    } catch (err) {
        // Silent error for database sync
    }
}

/**
 * Ekstrak teks balasan dari respons batchexecute Gemini.
 * Menggunakan beberapa fallback agar tidak crash jika struktur berubah.
 */
function extractReply(outerArray) {
    // === Fallback 1: Cari entry berdasarkan key "q4uTj", bukan index hardcoded ===
    let innerDataString = null;

    for (const item of outerArray) {
        if (Array.isArray(item) && item[1] === 'q4uTj' && item[2] != null) {
            innerDataString = item[2];
            break;
        }
    }

    // === Fallback 2: Coba outerArray[0][2] jika Fallback 1 gagal ===
    if (innerDataString == null && outerArray[0] && outerArray[0][2] != null) {
        innerDataString = outerArray[0][2];
    }

    // Jika masih null, lempar error yang jelas
    if (innerDataString == null) {
        throw new Error('innerDataString adalah null — format respons tidak dikenali');
    }

    // === Fallback 3: Parse array pembungkus lalu ambil objek candidates ===
    try {
        const innerDataArray = JSON.parse(innerDataString);
        const candidateRaw = Array.isArray(innerDataArray) ? innerDataArray[0] : innerDataArray;
        const resultObj = typeof candidateRaw === 'string' ? JSON.parse(candidateRaw) : candidateRaw;

        const text = resultObj?.candidates?.[0]?.content?.parts?.[0]?.text;
        if (text) return text;
    } catch (_) {
        // Lanjut ke fallback berikutnya
    }

    // === Fallback 4: Regex pada innerDataString ===
    const m = innerDataString.match(/"text"\s*:\s*"((?:[^"\\]|\\.)*)"/);
    if (m) {
        return m[1]
            .replace(/\\n/g, '\n')
            .replace(/\\"/g, '"')
            .replace(/\\\\/g, '\\');
    }

    // === Fallback 5: Regex pada string JSON mentah seluruh respons ===
    const rawStr = JSON.stringify(outerArray);
    const m2 = rawStr.match(/"text"\s*:\s*"((?:[^"\\]|\\.)*)"/);
    if (m2) {
        return m2[1]
            .replace(/\\n/g, '\n')
            .replace(/\\"/g, '"')
            .replace(/\\\\/g, '\\');
    }

    throw new Error('Semua metode ekstraksi gagal');
}

async function chat(promptText) {
    const jar = new CookieJar();
    const client = wrapper(axios.create({ 
        jar, 
        withCredentials: true,
        maxRedirects: 10 
    }));

    // 1. Load existing cookies from Firebase
    const savedCookies = await syncFirebase('GET');
    if (savedCookies) {
        try {
            const parsed = JSON.parse(savedCookies);
            if (Array.isArray(parsed)) {
                parsed.forEach(cookieStr => {
                    jar.setCookieSync(cookieStr, 'https://gemini.google.com');
                });
            } else {
                jar.setCookieSync(savedCookies, 'https://gemini.google.com');
            }
        } catch (e) {
            jar.setCookieSync(savedCookies, 'https://gemini.google.com');
        }
    }

    // 2. Construct RPC Payload
    const contents = {
        contents: [
            {
                role: 'user',
                parts: [{ text: promptText }]
            }
        ]
    };

    const rpcData = [
        null, 
        JSON.stringify(contents), 
        1, 
        'caea8d35955a'
    ];

    const fReq = [
        [
            [
                'q4uTj',
                JSON.stringify(rpcData),
                null,
                'generic'
            ]
        ]
    ];

    const payload = qs.stringify({
        'f.req': JSON.stringify(fReq),
        'at': '' 
    });

    try {
        // 3. Perform Request
        const response = await client.post(
            'https://gemini.google.com/_/BardChatUi/data/batchexecute',
            payload,
            {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
                    'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8',
                    'X-Same-Domain': '1',
                    'Origin': 'https://gemini.google.com',
                    'Referer': 'https://gemini.google.com/',
                    'Accept': '*/*'
                }
            }
        );

        // 4. Persistence: Capture and save new cookies
        const cookiesToSave = JSON.stringify(
            jar.getCookiesSync('https://gemini.google.com').map(c => c.toString())
        );
        await syncFirebase('PUT', cookiesToSave);

        // 5. Parse response
        let rawData = response.data;
        if (typeof rawData !== 'string') rawData = JSON.stringify(rawData);

        // Hapus prefix anti-XSS dari Google: )]}'
        const cleanData = rawData.replace(/^\)\]\}'\s*/m, '').trim();

        let outerArray;
        try {
            outerArray = JSON.parse(cleanData);
        } catch (e) {
            throw new Error(`Gagal parse outer JSON: ${e.message}`);
        }

        // 6. Ekstrak balasan dengan fallback berlapis
        const reply = extractReply(outerArray);
        return reply;

    } catch (err) {
        if (err.response?.status === 400) {
            throw new Error('Gemini Error 400: Payload rejected. Session might be invalid.');
        }
        throw new Error(`Gagal menghubungi Gemini V2: ${err.message}`);
    }
}

module.exports = { chat };
