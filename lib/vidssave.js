...
const CONFIG = {
    ...
    PROXY: "https://ricky01anjay-learning.hf.space/fetch?url="
};

/**
 * Helper to wrap URL with proxy
 */
const getProxiedUrl = (url) => `${CONFIG.PROXY}${encodeURIComponent(url)}`;
...
async function parseVideo(videoUrl) {
    ...
    try {
        const targetUrl = `${CONFIG.BASE_API}/media/parse`;
        const response = await axios.post(getProxiedUrl(targetUrl), payload, { 
            headers: HEADERS,
            timeout: 20000 
        });
...
async function createDownloadTask(resourceContent) {
    ...
    const targetUrl = `${CONFIG.BASE_API}/media/download`;
    const response = await axios.post(getProxiedUrl(targetUrl), payload, { headers: HEADERS });
    if (!response.data || !response.data.data) {
...
function monitorTask(taskId) {
    return new Promise((resolve, reject) => {
        const targetUrl = `${CONFIG.BASE_SSE}/media/download_query?auth=${CONFIG.AUTH}&domain=${CONFIG.DOMAIN}&task_id=${encodeURIComponent(taskId)}&download_domain=vidssave.com&origin=content_site`;
        const es = new EventSource(getProxiedUrl(targetUrl), { headers: HEADERS });
